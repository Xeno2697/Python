...
class Red: 
    ...
    def direction_reversal(self):
        self.back = True
        self.move_vectol *= -1
    def move_random(self,field:Field, vectol = np.zeros(2), alpha = RED_MOVE_Alpha):
    ...
    def action_mover(self, field:Field, search_data, alpha = RED_MOVE_Alpha):
        ...
        if(n < 3):
            ...
            #標準偏差を利用したランダム方向移動
        vectol = vec_sum/n
        #m = np.linalg.norm(vectol) * 0.91
        if(random.random() < 0.3 and not self.back):
                ...
                if(dis == 0):
                    ...
                    angle = random.gauss(rad, math.pi/2*alpha)#/m/m)
                self.rotate(angle)
    ...
    def action_anker(self, path:Path, search_data):
        ...
        if(self.number_to_container != 0):
            #アンカー番号は、減ることはあっても、本来増えることはないべき
            self.number_to_container = 99
            min_num = 99
            for i in range(len(search_data)):
            ...
            if(norm <= 0.0 and self.num == 0 and min_num != 0):
                self.time+=1
                if(self.time > (self.number_to_container * 15 + 30)):#撤退条件
                    self.mode = 2
                    self.time = 0
                    return 2
            else:
